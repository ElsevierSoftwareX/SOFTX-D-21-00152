...
begin
 try
  ...
  CBox.Caption:='';
  CBox.Width:=30;
//  CBox.Hint:='Выбрать/Исключить структурный уровень из поиска';
  AddCheckBox:=T+CBox.Height+3;
  if Ex then CBox.Checked:=True
   else CBox.Checked:=False;
  except
   AddCheckBox:=T;
   ShowMessage('Ошибка при создании CheckBox-а');
  end;
end;
...
begin
  try
   ...
   deltaY:=RoundTo((tmP.Height-4)/100,-2); //цена 1 деления
   deltaX:=Round((tmP.Width-4)/(vl.Count-1)); //ширина 1 сегмента
...
begin
 try
  ...
  if Cm is TRzGroupBox then //для rz групп
   begin
  ...
  if Cm is TRzMenuButton then //для кнопок
   begin
  ...
  if Cm is TToolButton then //для кнопок
   begin
