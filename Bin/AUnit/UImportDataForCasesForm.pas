...
begin
 ...
 if (Delimeter='tabstop')or(Delimeter='табуляция') then Delimeter:=#9;
...
if ComboBox3.ItemIndex<>-1 then
 ...
 cnt:=0;
  ...
  if (D='tabstop')or(D='табуляция') then D:=#9;
         ...
         for j := 0 to RzStringGrid2.ColCount-1 do
          begin
           ...
           if ((pos('Name',s2)=0)and(pos('Naimenovanie',s2)=0)and
            (pos('Nazvanie',s2)=0)and(pos('Имя',s2)=0)and
             (pos('Наименование',s2)=0)) then
               begin
                 tmSlot:=TSlot.Create;
                 tmSlot.Name:=s2;
  ...
  //           begin
               ...
               if tmT.IndexOf(tmSlot.ShortName,-1)=-1 then
                tmT.Slots.Add(tmSlot)
                  ...
                  //and we add another сopy of the template
                  tmTs.AddObject(tmT.Name,tmT);
  //           end;
            end;
          end;
